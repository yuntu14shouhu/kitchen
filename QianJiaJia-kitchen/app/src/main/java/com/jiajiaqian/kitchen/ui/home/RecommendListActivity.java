...
public class RecommendListActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void initData(Bundle savedInstanceState) {
        if (getIntent().getStringExtra(KitchenConstants.PRODUCT_LIST_INTENT_KEY) != null
                && getIntent().getStringExtra(KitchenConstants.PRODUCT_LIST_INTENT_KEY).equals(KitchenConstants.ProductListIntentValue.RECOMMEND_LIST)) {
            mSortNameTv.setText("推荐商品");
        }
        KitchenHttpManager.getInstance().getRenQiLists(new OkJsonRequest.OKResponseCallback() {
            @Override
            public void onResponse(JSONObject jsonObject) {
                ...
                if (productListBean.getData() != null && productListBean.getData().size() > 0) {
                    DiscountListAdapter discountListAdapter = new DiscountListAdapter(productListBean.getData(), R.layout.listitem_subsort_list, RecommendListActivity.this);
                    mRecommendListView.setAdapter(discountListAdapter);
                }
            }

            @Override
            public void onErrorResponse(VolleyError volleyError) {
                Log.e("recommend_list_error","volleyError: "+volleyError.getMessage());
            }
        });
    }
    ...
    public void initView() {
        ...
        mRecommendListView = (RecyclerView) this.findViewById(R.id.rcy_sub_sort);
        mRecommendListView.setLayoutManager(new LinearLayoutManager(this));
        mRecommendListView.setItemAnimator(new DefaultItemAnimator());
        mRecommendListView.setHasFixedSize(true);
    }
